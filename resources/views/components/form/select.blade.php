...
<div x-data="{
        ...
        selectHighlighted() {
            if (this.options.length > 0 && this.highlightedIndex >= 0 && this.highlightedIndex < this.options.length) {
                this.select(this.options[this.highlightedIndex].id);
            }
        },
        scrollToHighlighted() {
            this.$nextTick(() => {
        ...
        toggle() {
            ...
            if (this.open) {
                this.highlightedIndex = 0;
                // Find index of selected item if any
                if (this.selected) {
    ...
    }" class="relative" @click.away="open = false" @keydown.escape="open = false"
    @keydown.arrow-down.prevent="highlightNext()" @keydown.arrow-up.prevent="highlightPrev()"
    @keydown.enter.prevent="toggle(); if(!open) selectHighlighted()">
    ...
    <div x-show="open" style="display: none;" x-transition:leave="transition ease-in duration-100"
        ...
        <!-- Options List -->
        <ul class="py-1 overflow-y-auto max-h-60" x-ref="optionsList">
            ...
            <li x-show="options.length === 0" class="px-3 py-2 text-gray-500">
